# image_processing.py # ! Still have some errors: broken when use canny on color image
# ? Should: replace adjustContrast by enableContrastAdjustment, adjustBrightness ...
# ! Should: add check whether file correct format
# ? Qt has save-file func?
# ? Should: split convertCVToQImage into convertCVToQImage and displayCVImage(displayCVImage contains convertCVToQImage)
...
from PyQt5.QtWidgets import (
    ...
    QSpinBox, QDoubleSpinBox, QFrame, QFileDialog, QMessageBox, QHBoxLayout,
    QVBoxLayout, QAction)
...
class ImageProcessingGUI(QMainWindow):
    ...
    def initializeUI(self):
        """Initialize the window and display its contents to ther screen.
        """
        print(f'\ninitializeUI')
    ...
    def setupWindow(self):
        ...
        main_h_box.addWidget(self.image_label, 1)  # ? what 1 mean?
        main_h_box.addWidget(side_panel_frame)

        # Create container widget and set main windows's widget
        container = QWidget()
    ...
    def edgeDetection(self, state):
        """The slot corresponding to applying edget detection.
    ...
    def applyImageProcessing(self):
        """For the boolean variables related to the image processing techniques, 
        if True, apply the corresponding process to the image 
    ...
    def openImageFile(self):
        ...
        image_file, _ = QFileDialog.getOpenFileName(
            self, "Open Image", os.getenv('HOME'),
            "Images (*.png *.jpeg *.jpg *.bmp)")  # ! Should add: check whether file correct format
        ...
        if image_file:
            ...
            # A copy of the original image # ? Mat type can copy data by "="?
            self.copy_cv_image = self.cv_image
            # Create a destination image for the contrast and brightness processes
            self.processed_cv_image = np.zeros(
                self.cv_image.shape, self.cv_image.dtype)
            self.convertCVToQImage(self.cv_image)
        else:
    ...
    def saveImageFile(self):
        ...
        image_file, _ = QFileDialog.getSaveFileName(
            self, "Save Image", os.getenv('HOME'),
    ...
    def convertCVToQImage(self, image):
        """Load a cv image and convert the image to a Qt QImage. 
        Display the image in image_label.
        ...
        # Number of bytes required by the image pixels in a row;
        # dependency on the number of channels
        bytes_per_line = width * channels

        converted_Qt_image = QImage(
            image, width, height, bytes_per_line, QImage.Format.Format_RGB888)
